...
public class UriEnhanced {
    ...
    private Map<KeyIndependent, String> queryProperties = new HashMap<>();
    ...
    public static UriEnhanced createFromString(String s) throws CastException {

        if (s == null) {
            throw new CastException("The string cannot be null");
        }
        URI uri;
        try {
            uri = URI.create(s);
            ...
            try {
                ...
                throw new CastException("Illegal URI, URL or encoding: " + e.getMessage(), e);
            }
        }
    ...
    public static UriEnhanced createFromUri(URI uri) throws CastException {
     ...
     */
    public UriEnhanced setHost(String host) throws CastException {
        if (host == null) {
            return this;
        }
        if (host.contains(":")) {
        ...
        try {
            ...
            throw new CastException("The host value (" + host + ") is not a valid domain name", e);
        }
        return this;
    }
     ...
     * @return the object for chaining
     */
    private UriEnhanced setAuthority(String authority) throws CastException {
        String[] authorityParts = authority.split(":");
        this.setHost(authorityParts[0]);
        if (authorityParts.length >= 2) {
            String authorityPort = authorityParts[1];
            try {
                this.port = Integer.parseInt(authorityPort);
            } catch (NumberFormatException e) {
                throw new CastException("The authority port (" + authorityPort + ") of (" + authority + ") is not an integer");
            }
        }
        return this;
    }
    ...
    public UriEnhanced addQueryProperty(String key, String value) {
        this.queryProperties.put(new KeyIndependent(key), value);
        return this;
    }
     ...
     * @throws CastException - if the URI is not a valid origin
     */
    @SuppressWarnings("unused")
    public UriEnhanced validateAsOrigin() throws CastException {
        if (!this.queryProperties.isEmpty()) {
            throw new CastException("An origin URI does not have any query properties");
        }
        if (fragment != null) {
            throw new CastException("An origin URI does not have any fragment");
        }
        return this;
    }
    ...
    private String getQueryString(boolean withEncoding) {
        ...
        for (Map.Entry<KeyIndependent, String> queryProperty : this.queryProperties.entrySet()) {
            if (firstIsPassed) {
                query.append(AMPERSAND_CHARACTER);
            } else {
                firstIsPassed = true;
            }
            String key = queryProperty.getKey().toString();
            if (withEncoding) {
     ...
     */
    protected static Map<KeyIndependent, String> getQueryAsMap(String query) {
        if (query == null) {
            return new HashMap<>();
        }
        String[] queryParts = query.split(UriEnhanced.AMPERSAND_CHARACTER);
        HashMap<KeyIndependent, String> parameters = new HashMap<KeyIndependent, String>();
        for (String queryPart : queryParts) {
            ...
            KeyIndependent keyIn = new KeyIndependent(key);
            parameters.put(keyIn, value);
        }
        return parameters;
    }
